...
def sheet_a():
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name(
        'Test-sheet-e432f87de6bc.json', scope)
    client = gspread.authorize(creds)
    sheet = client.open('Tut').sheet1

    html_text = requests.get(
        f'https://www.timesjobs.com/candidate/job-search.html?searchType=personalizedSearch&from=submit&txtKeywords={fs}&txtLocation=').text
    soup = BeautifulSoup(html_text, 'lxml')
    js = soup.find_all('li', class_='clearfix job-bx wht-shd-bx')
    for index, j in enumerate(js):
        jf = j.find('span', class_='sim-posted').span.text
